...
class EfficientImplementation:
    ...
    def compute_gradients(self) -> tuple:
        ...
        # Final loss gradient
        loss = 0.5 * torch.sum((x - self.x_target)**2)
        dL_dx = x - self.x_target  # [state_dim]
        ...
        accumulated_grad = dL_dx  # [state_dim]
        
        for t in range(self.time_steps-1, -1, -1):
            A, B, C, D = derivatives[t]
            # A: [state_dim, control_dim]
            # B: [control_dim, param_dim]
            # C: [state_dim, state_dim]
            # D: [control_dim, state_dim]
            
            # Compute gradient contribution at this timestep
            control_grad = A.T @ accumulated_grad  # [control_dim]
            param_grad = B.T @ control_grad  # [param_dim]
            ...
            if t > 0:
                ...
                control_to_state = D.T @ control_grad  # [state_dim]
    ...
    def compute_ABCD(self, x: torch.Tensor, x_next: torch.Tensor, c: torch.Tensor) -> tuple:
        """
        Compute Jacobian matrices with proper dimensions using functional.jacobian:
